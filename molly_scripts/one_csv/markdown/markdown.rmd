...
one_csv %>%
  drop_na() %>%
  mutate(new_name= ifelse(data_type=="aa",paste0(name,".aa"),name)) %>%
  filter(ic_type == "AIC") %>%
  select(new_name, data_type, number_models) %>%
  separate(new_name, into = c("keep1", "keep2", "keep3", "bye1"), sep = "\\.") %>%
  select(-bye1) %>%
  unite(new_name, c("keep1", "keep2", "keep3")) %>%
  pivot_wider(names_from = data_type, values_from = number_models)
  

#pseudocode
#goal is to spread data_type with number_models by name of file
  #spread by data_type with number_models
  #separate by name of file
  # move around vars then merge via name after work is done 
...
spread_csv %>%
  ggplot(aes(x=aa,y=nt)) +
  geom_hex(color="black",bins=5) +
  scale_fill_gradientn(colours=c("goldenrod","skyblue")) +
  scale_x_continuous(name="Number of AA Models") +
  scale_y_continuous(name="Number of NT Models") 


#fiddle make it look good !

...
## Actual modeling of n_models/matrices and stability---------------------------
...
same_as_rep50 %>%
  ...
  filter(between(count_model_sameas_rep50, 3, 47)) %>%
  select(id, datatype, ic_type) %>%
  inner_join(same_as_rep50) -> rep50_data_for_lm

rep50_data_for_lm %>%
  select(id, datatype, ic_type) %>%
  distinct() %>%
  tally() # 5650 situations are able to be modeled here.

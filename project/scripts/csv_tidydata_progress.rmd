...
csv_dataframe <- read_csv("allcsvs.csv")
...
csv_dataframe %>%
  ...
  select(-filename,-STOP) %>%
  group_by(species) %>%
  extract(species,c("species1","species2"), "(^D), (^E)") %>%
  ungroup()
...
I've tried using two functions to separate the `species` column into two columns, by the two species specified. The function from dplyr `separate()` is being used to try to separate and then I tried using `extract()`. I think the main problem I'm having is using regular expressions.

So, after I figure out the regular expressions, I will try to separate the .csv by the data type.

Maybe I'll try this in Python? Probably not because that would be too complicated.

...
nt_trees = os.listdir(path_to_nt_output_trees)
...
nt_unaligned_files = os.listdir(nt_unaligned_dir)
...
for file in aa_unaligned_files:
    if file.endswith(file_ending2):
        ...
        aa_outfile.write(aa_output_string + aa_output_string2)
    else:
        continue
        
### NT DATA
for tree in nt_trees:
    nt_path = path_to_nt_output_trees + tree
    if os.path.exists(nt_path):
        try:
            nt_treefile = path_to_nt_output_trees + tree
            nt_tree = dendropy.Tree.get(
                path=nt_treefile,
                schema="newick") 
        
            nt_treelength = nt_tree.length()
            #print(aa_treelength)
            #print("----------------------------------------------------------------")
            nt_pdc = nt_tree.phylogenetic_distance_matrix()
            nt_meanpairwise = nt_pdc.mean_pairwise_distance()
            
            nt_output_string2 = str(nt_treelength) + comma + str(nt_meanpairwise) + "\n"
            #print(aa_output_string2)
...
#Other data code       
for file in nt_unaligned_files:
    if file.endswith(file_ending2):
        #collect information
        name = file.rstrip(file_ending2)
        #print("name:", name)
        fasta_records = list(SeqIO.parse(nt_unaligned_dir + file, "fasta"))
        #print(fasta_records)
        numseq = len(fasta_records)
        #print(numseq)
        ## MORE INFORMATION IS COLLECTED HERE ###
        ...
        #print(aa_output_string)
        nt_outfile.write(nt_output_string + nt_output_string2)
    else:
        continue
